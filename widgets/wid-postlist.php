...
class d_postlist extends WP_Widget
{
    function d_postlist()
    {
        ...
        $this->WP_Widget('d_postlist', 'mytheme-聚合文章', $widget_ops, $control_ops);
    }
        ...
        <p>
            <label>
                ...
                <input style="width:100%;" id="<?php echo $this->get_field_id('cat'); ?>"
                       ...
                       value="<?php echo $instance['cat']; ?>" size="24"/>
            </label>
        </p>
        <p>
            <label>
                ...
                <input style="width:100%;" id="<?php echo $this->get_field_id('limit'); ?>"
                       ...
                       value="<?php echo $instance['limit']; ?>" size="24"/>
            </label>
        </p>
        <p>
            <label>
                More 显示文字：
                <input style="width:100%;" id="<?php echo $this->get_field_id('more'); ?>"
                       name="<?php echo $this->get_field_name('more'); ?>" type="text"
                       value="<?php echo $instance['more']; ?>" size="24"/>
            </label>
        </p>
        <p>
            <label>
                More 链接：
                <input style="width:100%;" id="<?php echo $this->get_field_id('link'); ?>"
                       name="<?php echo $this->get_field_name('link'); ?>" type="url"
                       value="<?php echo $instance['link']; ?>" size="24"/>
            </label>
        </p>
        <p>
            <label>
                <input style="vertical-align:-3px;margin-right:4px;" class="checkbox"
                       type="checkbox" <?php checked($instance['img'], 'on'); ?>
                       id="<?php echo $this->get_field_id('img'); ?>"
                       name="<?php echo $this->get_field_name('img'); ?>">显示图片
            </label>
        </p>
...
function dtheme_posts_list($orderby, $limit, $cat, $img)
{
    ...
    while (have_posts()) : the_post();
        ?>
        <li>
            <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php if ($img) {
            ...
            &nbsp;•&nbsp;<!--<i class="fa fa-comments-o"></i>--><?php /*comments_number('0评论', '1评论', '%评论'); */ ?>
            <span class="postlist_comments">
            <?php
