        ...
        <div class="content">
           <?php if (dopt('d_singleMenu_b')) echo '<div class="breadcrumbs">' . deel_breadcrumbs() . '</div>'; ?>
            ...
            <?php while (have_posts()) : the_post(); ?>
                <header class="article-header">
                    ...
                    <div class="meta">
                        ...
                        <time class="muted"><i
                                class="fa fa-clock-o"></i> <?php echo get_the_time('Y-m-d G:i') ?>
                        </time>
                ...
                <?php if (user_is_mobile()): ?><?php if (dopt('Mobiled_adpost_01_b')) echo '<div class="banner-post">' . dopt('Mobiled_adpost_01') . '</div>'; ?><?php endif; ?>
                <article class="article-content">
                    ...
                    <div class="article-social">
                        <a href="javascript:;" data-action="ding" data-id="<?php the_ID(); ?>" id="Addlike"
                           class="action<?php if (isset($_COOKIE['bigfa_ding_' . $post->ID])) echo ' actived'; ?>"><i
                                class="fa fa-heart-o"></i>喜欢 (<span
                                class="count"><?php if (get_post_meta($post->ID, 'bigfa_ding', true)) {
                                    echo get_post_meta($post->ID, 'bigfa_ding', true);
                                } else {
                                    echo '0';
                                } ?></span>)</a><?php if (dopt('d_bdshare_b')) echo '<span class="or">or</span>';
                        deel_share(); ?>
                    </div>
                </article>
            <?php endwhile; ?>
            ...
            <nav class="article-nav">
                <span
                    class="article-nav-prev"><?php previous_post_link('<i class="fa fa-angle-double-left"></i> %link'); ?></span>
                <span
                    class="article-nav-next"><?php next_post_link('%link  <i class="fa fa-angle-double-right"></i>'); ?></span>
            </nav>

            <div class="related_top">
                <?php include('modules/related.php'); ?>
            </div>
            <?php if (user_is_mobile()): ?><?php if (dopt('Mobiled_adpost_02_b')) echo '<div id="comment-ad" class="banner-related">' . dopt('Mobiled_adpost_02') . '</div>'; ?><?php endif; ?>
