...
<?php } ?>
<?php if (user_is_mobile()): ?><?php if (dopt('Mobiled_adindex_02_b')) printf('<div class="banner-sticky">' . dopt('Mobiled_adindex_02') . '</div>'); ?><?php endif; ?>
<?php
...
<?php while (have_posts()) : the_post(); ?>
    ...
    <article class="excerpt<?php echo !$_thumbnail ? ' excerpt-nothumbnail' : '' ?>">
        <?php if(!user_is_mobile()) {?>
            <?php if ($_thumbnail) { ?>
                <div class="focus">
                    <a target="_blank" href="<?php the_permalink(); ?>">
                        <img class="thumb" src="<?php echo get_bloginfo("template_url") ?>/timthumb.php?src=<?php echo post_thumbnail_src(); ?>&h=120&w=160&q=90&zc=1&ct=1"
                             alt="<?php the_title(); ?>"/>
                    </a>
                </div>
            <?php } ?>
            ...
            <span class="note">
                <?php echo deel_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 150, '&nbsp...');?>
            </span>
            <p class="auth-span">
                ...
                <?php } ?>
                <?php if (!$_time) { ?><span class="muted"><i
                    class="fa fa-clock-o"></i> <?php echo get_the_time('Y-m-d G:i') ?>
                    </span><?php } ?>
                <?php if (!$_views) { ?><span class="muted"><i class="fa fa-eye"></i> <?php deel_views('℃'); ?>
                    </span><?php } ?>
